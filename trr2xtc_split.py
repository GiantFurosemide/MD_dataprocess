...
for trr in trr_files:
	...
	print(cmd)
	os.system(cmd)
	cmd = f'echo 1 0|gmx trjconv -f {outname} -s {gro_name} -pbc nojump -center -o {filename}_nojump.xtc'
	print(cmd)
	os.system(cmd)
	cmd = f'echo 1 1 0|gmx trjconv -f {filename}_nojump.xtc -s {gro_name} -center -o {filename}_nojump_rottrans.xtc -fit rot+trans && rm -v {filename}_nojump.xtc'
	print(cmd)
	os.system(cmd)
