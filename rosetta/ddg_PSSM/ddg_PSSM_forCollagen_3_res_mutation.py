...
This a script for 3 chains protein, to mutate each residue to all 20 amino acids and calculate the ddg value.
same mutation will be performed on 3 chains in the same time.
...
def mutate_residue(input_pose, res_index, new_aa, res_label = None):
    ...
    <MOVERS>
      ...
      <MutateResidue name="mutate_18" new_res="%(new_aa)s" target="%(res_index_18)i" />
      <MutateResidue name="mutate_36" new_res="%(new_aa)s" target="%(res_index_36)i" />
    </MOVERS>
    <PROTOCOLS>
      <Add mover_name="mutate"/>
      <Add mover_name="mutate_18"/>
      <Add mover_name="mutate_36"/>
    </PROTOCOLS>
    </ROSETTASCRIPTS>
    """ % {
      'new_aa': new_aa,
      'res_index': res_index,
      'res_index_18': res_index + 18,
      'res_index_36': res_index + 36
    }
